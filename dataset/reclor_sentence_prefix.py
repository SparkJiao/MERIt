...
from dataset.data_utils import get_sep_tokens, is_bpe, get_unused_tokens
from general_util.logger import get_child_logger

logger = get_child_logger("ReClor.Sentence.Prefix")
...
def _convert_example_to_features(example: Dict, tokenizer: PreTrainedTokenizer, max_seq_length: int, token_num: int = 4) -> Dict:
    ...
    for option in example["options"]:
        ...
        lens_to_remove = len(context_tokens) + len(pet_question_tokens) + len(opt_tokens) + len(sep_tokens) + (
        ...
        tru_c_tokens, tru_q_o_tokens, _ = tokenizer.truncate_sequences(context_tokens,
                                                                       pet_question_tokens + sep_tokens + opt_tokens,
...
def convert_examples_into_features(file_path: str, tokenizer: PreTrainedTokenizer, max_seq_length: int,
    ...
    with Pool(num_workers) as p:
        _annotate = partial(_convert_example_to_features, tokenizer=tokenizer, max_seq_length=max_seq_length, token_num=token_num)
        features = list(tqdm(
