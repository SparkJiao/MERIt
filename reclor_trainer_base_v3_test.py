...
from fairscale.nn.wrap.auto_wrap import auto_wrap, enable_wrap, default_auto_wrap_policy, wrap
...
def save_model(model: Union[torch.nn.Module, FullyShardedDP], cfg: DictConfig, output_dir: str, tokenizer: PreTrainedTokenizer = None):
...
def train(cfg, train_dataset, features, model, tokenizer, continue_from_global_step=0):
    ...
    if cfg.local_rank != -1:
        ...
        fsdp_params = dict(mixed_precision=cfg.fp16,
                           flatten_parameters=getattr(cfg, "flatten_parameters", True),
                           reshard_after_forward=cfg.reshard_after_forward,
                           move_grads_to_cpu=cfg.move_grads_to_cpu,
                           move_params_to_cpu=cfg.move_params_to_cpu)
