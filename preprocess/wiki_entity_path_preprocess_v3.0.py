...
def process_path(path: Tuple, sentences, entities, sent2ent):
    ...
    for e_id, e_sent_id in path:
        selected_sentences[e_sent_id]["ent"].extend([e for e in entities[e_id] if e["sent_id"] == e_sent_id])
    ...
    for s_id in rest_sentences:
        for e_id in sent2ent[s_id]:
            rest_sentences[s_id]["ent"].extend([e for e in entities[e_id] if e["sent_id"] == s_id])
                ...
                pos = [
                    {
                        ...
                        "h": [e for e in entities[h] if e["sent_id"] == pos_sent_id],
                        "t": [e for e in entities[t] if e["sent_id"] == pos_sent_id],
