...
class ReClorSentenceMLMCollator:
    def __call__(self, batch: List[Tuple[Tensor, ...]]) -> Dict[str, Tensor]:
        if len(batch[0]) == 6:
            input_ids, attention_mask, token_type_ids, labels, sentence_spans, mlm_labels = list(zip(*batch))
        elif len(batch[0]) == 5:
            input_ids, attention_mask, labels, sentence_spans, mlm_labels = list(zip(*batch))
            token_type_ids = None
        else:
            raise RuntimeError()
        ...
        mlm_labels = torch.stack(mlm_labels, dim=0)
