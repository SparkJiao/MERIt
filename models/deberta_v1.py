...
from transformers.models.deberta.modeling_deberta import DebertaPreTrainedModel, DebertaModel, DebertaConfig, \
    ContextPooler, StableDropout, ACT2FN, DebertaEncoder
...
def masked_layer_norm(layer_norm_layer, inputs, mask=None):
...
class DebertaForMultipleChoicePreTrain(DebertaPreTrainedModel, LogMixin, ABC):
    def __init__(self, config: DebertaConfig,
        ...
        if use_stable_embedding:
            ...
            self.deberta.embeddings.word_embeddings = bnb.nn.StableEmbedding(config.vocab_size, self.embedding_size,
        ...
        if fs_checkpoint:
            for i in range(fs_checkpoint_start_layer_id, config.num_hidden_layers):
                self.deberta.encoder.layer[i] = checkpoint_wrapper(self.deberta.encoder.layer[i],
