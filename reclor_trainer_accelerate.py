...
"""
- This is a dev version for DeepSpeed using accelerate.
- The git repo of accelerate is: https://github.com/huggingface/accelerate/commit/42cb31c107058ce1bb0b32036e41d28a42eaa2d7
  with version 0.4.0.dev0


TODO: Some problems have not been fixed.
"""
...
def save_model(model: torch.nn.Module, accelerator: Accelerator, cfg: DictConfig, output_dir: str, tokenizer: PreTrainedTokenizer = None):
...
def train(cfg, accelerator: Accelerator, train_dataset, features, model, tokenizer, continue_from_global_step=0):
    ...
    # eval_dataloader = load_eval_data(cfg, tokenizer, _split="dev")
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % cfg.gradient_accumulation_steps == 0:
                ...
                if cfg.save_steps > 0 and global_step % cfg.save_steps == 0:
                    ...
                    save_model(model, accelerator, cfg, output_dir, tokenizer)
...
@hydra.main(config_path="conf/accelerate", config_name="config")
def main(cfg: DictConfig) -> None:
    deepspeed_plugin = DeepSpeedPlugin(zero_stage=2, gradient_accumulation_steps=cfg.gradient_accumulation_steps,
                                       offload_optimizer_device='cpu')
    # cfg.gradient_accumulation_steps = 1

    accelerator = Accelerator(fp16=cfg.fp16, deepspeed_plugin=deepspeed_plugin)
    cfg.local_rank = accelerator.local_process_index if accelerator.distributed_type != "NO" else -1
    ...
    logger.warning("Process rank: %s, device: %s, n_gpu: %s, distributed training: %s, 16-bits training: %s",
                   cfg.local_rank, cfg.device, "null", bool(cfg.local_rank != -1), cfg.fp16)
